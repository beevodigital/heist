...
class BaseGameMode(procgame.game.Mode):
	...
	##switch points
	def sw_popl_closed(self,sw):
		self.game.score(639)
	def sw_popr_closed(self,sw):
		self.game.score(639)
	def sw_popm_closed(self,sw):
		self.game.score(639)
	def sw_slingr_closed(self,sw):
		self.game.score(1123)
...
class HeistGame(procgame.game.BasicGame):
	...
	def ball_ended(self):
		...
		#needs to be updated set back to false
		self.enable_flippers(True)
